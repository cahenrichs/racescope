...
func NewWeekendPublisher(db TransactionStarter) *WeekendPublisher {
	return &WeekendPublisher{db: db}
}

func (p *WeekendPublisher) ReplaceWeekend(ctx context.Context, snapshot ingest.Snapshot, publishedAt time.Time) error {
	tx, err := p.db.Begin(ctx)
	if err != nil {
		return fmt.Errorf("begin weekend replacement: %w", err)
	}
	...
	err = tx.QueryRow(ctx, `
		...
		RETURNING id`, meeting.Season.PublicID, meeting.Season.Year, publishedAt).Scan(&seasonID)
	if err != nil {
	...
	err = tx.QueryRow(ctx, `
		...
		meeting.Circuit.CountryCode, meeting.Circuit.CountryName, meeting.Circuit.Location, publishedAt).Scan(&circuitID)
	if err != nil {
	...
	err = tx.QueryRow(ctx, `
		...
		meeting.OfficialName, meeting.DateStart, meeting.DateEnd, meeting.IsCancelled, publishedAt).Scan(&meetingID)
	if err != nil {
	...
	for _, session := range snapshot.Weekend.Sessions {
		...
		if !ok {
			return fmt.Errorf("session %q has no private source key", session.PublicID)
		}
		var id int64
		err := tx.QueryRow(ctx, `
			...
			session.Name, session.Type, session.DateStart, session.DateEnd, session.IsCancelled, publishedAt).Scan(&id)
		if err != nil {
			return fmt.Errorf("insert session %q: %w", session.PublicID, err)
		}
	...
	for _, driver := range snapshot.Drivers {
		...
		err := tx.QueryRow(ctx, `
			...
			ON CONFLICT (public_id) DO UPDATE SET first_name = EXCLUDED.first_name, last_name = EXCLUDED.last_name,
				...
				source_fetched_at = EXCLUDED.source_fetched_at, ingested_at = CURRENT_TIMESTAMP
			RETURNING id`, driver.PublicID, driver.FirstName, driver.LastName, driver.FullName, driver.NameAcronym, publishedAt).Scan(&id)
		if err != nil {
			return fmt.Errorf("upsert driver %q: %w", driver.PublicID, err)
		}
	...
	for _, constructor := range snapshot.Constructors {
		...
		err := tx.QueryRow(ctx, `
			...
			ON CONFLICT (public_id) DO UPDATE SET season_id = EXCLUDED.season_id, name = EXCLUDED.name,
				source_fetched_at = EXCLUDED.source_fetched_at, ingested_at = CURRENT_TIMESTAMP
			RETURNING id`, constructor.PublicID, seasonID, constructor.Name, publishedAt).Scan(&id)
		if err != nil {
			return fmt.Errorf("upsert constructor %q: %w", constructor.PublicID, err)
		}
	...
	for _, entry := range snapshot.Entries {
		...
		if !sessionOK || !driverOK || !constructorOK {
			return fmt.Errorf("entry %q references unpublished domain records", entry.PublicID)
		}
		var id int64
		err := tx.QueryRow(ctx, `
			...
			constructorID, entry.DriverNumber, entry.TeamColour, publishedAt).Scan(&id)
		if err != nil {
			return fmt.Errorf("insert entry %q: %w", entry.PublicID, err)
		}
	...
	for _, result := range snapshot.Results {
		...
		if !ok {
			return fmt.Errorf("result %q references an unpublished entry", result.PublicID)
		}
		...
		_, err := tx.Exec(ctx, `
			...
			gapNumber, gapText, gapNumbers, result.GapToLeader.Kind, result.SourceOrder, publishedAt)
		if err != nil {
			return fmt.Errorf("insert result %q: %w", result.PublicID, err)
		}
	}

	if err := tx.Commit(ctx); err != nil {
		return fmt.Errorf("commit weekend replacement: %w", err)
	}
	return nil
}
